...
#
# for each time period, pull only relevant obs and locations and join by hexagon

### First for summer (1st) time period
obsIn <- obsSumm
locIn <- locSumm
res <- hex
sppNout <- as.list(NA)
...
# and once each time period has been run, save as new object

resSumm <- res


### and again for second time period:
...
st_write(resSumm,"../Data_Processed/resultsSumm_v2.shp")
st_write(resFall,"../Data_Processed/resultsFall_v2.shp")
